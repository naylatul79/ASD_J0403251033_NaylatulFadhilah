...
def partition(data,first,last):
    ...
    while not done:
        ...
        while leftmark <= rightmark and data[leftmark] >= pivotvalue:  # descending: cari nilai yang lebih kecil dari pivot (tanda <= diganti dengan >=)
            leftmark = leftmark + 1

        while data[rightmark] <= pivotvalue and rightmark >= leftmark:  # descending: cari nilai yang lebih besar dari pivot (tanda >= diganti dengan <=)
